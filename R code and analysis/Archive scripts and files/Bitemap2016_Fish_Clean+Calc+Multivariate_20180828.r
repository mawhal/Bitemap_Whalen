...
# have yet to here from several sites, for now assume default conditions (10m wide transect, pull to shore in 100cm)
seines.raw %>% 
  select( Country ) %>%
  distinct() 
...
rate.mean <- ddply( rate.env, .(Country,  habitat), summarise, rate=mean( rate,na.rm=TRUE ) )
rate.mean2 <- ddply( rate.env, .(Country), summarise, rate=mean( rate,na.rm=TRUE ) )
...
no.squid <- c( "Pleuronectidae", "Paralichthyidae", "Rhombosoleidae", "Hippolytidae", 
               ...
               "Stichaeidae","Triglidae","Batrachoididae","Platycephalidae","Synodontidae" ) # last five added 28 December 2018
# likely to eat squid
...
fam.pres <- trait.rate %>%
  filter( eat.squid==1 ) %>%
  select( Site,method,hemi,basin,coast,meanLat,sstmean,rate,family,totalCount ) %>%
...
fam.cap <- capscale( y ~ sstmean, x, dist="raup")
fam.rda <- dbrda( y ~ sstmean, x, dist="raup")
...
windows(10,4)
plot_grid(centroids,sstsurf, labels=c("",""), ncol = 2, nrow =1,label_size=18, rel_widths = c(1,1.25) )
...
selected <- c( 'Blenniidae', 'Clinidae', 'Cottidae', 'Fundulidae', 'Gadidae', 'Gerreidae', 'Haemulidae', 'Hemiramphidae', 'Hexagrammidae',
...
glm1 <- lme4::glmer( rate) # account for Site variation (Country) and fit independently for each family (wish I could combine into a single analysis)
...
ggplot(pred.all, aes(x=habitat, y=log10(cpua), fill = habitat)) + 
  geom_boxplot(width=0.5,notch=TRUE) +
  geom_point(alpha=0.1) +
  theme_classic() +
  theme(axis.text=element_text(size=12),
...
# graph fish diversity by habitat type
# windows(3,3)
ggplot(pred.all, aes(x=habitat, y=biomass.area+0.01, fill = habitat)) + 
  geom_boxplot(width=0.5,notch=TRUE) +
  geom_point(alpha=0.1) +
  theme_classic() +
  theme(axis.text=element_text(size=12),
...
write.csv(  pred.all, "Output Data/Bitemap_SEINE_summaries_20190107.csv", row.names=FALSE )
write.csv(  sites.nmds, "Output Data/Bitemap_BioORACLE_20190107.csv", row.names = FALSE )
