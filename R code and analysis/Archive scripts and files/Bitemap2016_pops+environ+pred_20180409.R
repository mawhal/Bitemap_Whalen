...
## PREDATOR DATA
predator <- read.csv( "Bitemap_SEINE_summaries_20180413.csv", stringsAsFactors = FALSE )
...
sites <- read.csv( "Output Data/Bitemap_BioORACLE_2018.04.18.csv", stringsAsFactors = FALSE )
...
dev.off()
with(sites, plot( abs(meanLat)~parmean) )
...
ggplot(rate.env, aes(x=meanLat, y=rate, col=habitat, fill=habitat ))  +
  # geom_smooth(se=F)  +
  geom_smooth( aes(group=1), se=F, col='black', lwd=0.5, lty=2 ) +
  geom_smooth(aes(group=hemi), se=F, col='black', lwd=0.5 ) +
  geom_point(pch=21,size=1.5,alpha=0.5) +
  scale_fill_manual(values=c(unveg.color,seagrass.color)) +
  scale_color_manual(values=c(unveg.color,'black')) +
  theme_classic() + 
...
ggplot(rate.env, aes(x=abLat, y=rate, group=hemi )) + facet_grid(hemi~basin) +  
...
rate.site <- ddply( rate.env, .(Country,Site,hemi,basin,coast), summarise, rate=mean(rate), meanLat=mean(abLat), sstmean=mean(sstmean) )
windows(9,5)
ggplot(rate.site, aes(x=sstmean, y=rate, group=hemi )) + facet_grid(hemi+coast~basin) +  
...
dna <- rate.env[ rate.env$hemi == "North" & rate.env$basin == "Atlantic" & rate.env$coast == "West", ]
  range(dna$Lat)
...
m3na <- glmer( rate ~ poly(abLat,3) + (1|Country), dna, family="binomial" )
anova(m1na,m2na)
anova(m2na,m3na)

m1sp <- glmer( rate ~ poly(abLat,1) + (1|Country), dsp, family="binomial" ) 
m2sp <- glmer( rate ~ poly(abLat,2) + (1|Country), dsp, family="binomial" ) 
m3sp <- glmer( rate ~ poly(abLat,3) + (1|Country), dsp, family="binomial" ) 
m4sp <- glmer( rate ~ poly(abLat,4) + (1|Country), dsp, family="binomial" ) 
anova(m1sp,m2sp)
anova(m2sp,m3sp)
anova(m3sp,m4sp)
#
...
ggplot(rate.env, aes(x=Swarm, y=rate )) + geom_point(alpha=0.1) +
  geom_smooth(method='glm',method.args=list(family=quasibinomial)) + theme_classic()
ggplot(rate.env, aes(x=Swarm, y=logbio )) + geom_point(alpha=0.1) +
  geom_smooth(method='glm',method.args=list(family=quasibinomial)) + theme_classic()
...
bio12 <- glmer( rate~ Swarm + temp + (1|Country), rateclean, family='binomial' )
bio13 <- glmer( rate~ Swarm + temp + (1|Country), rateclean, family='binomial' )

AICctab( m0, latm3, tempm1, sstm3, sstm4, sstm5, bio1, bio2, bio3, bio4, 
         bio5, bio6, bio7,bio8,bio9,bio10,bio11, bio12, nobs=nrow(rateclean) )
...
ggplot( rateclean, aes(x=temp,y=rate)) + geom_point() + geom_smooth(method='lm')
