...
## PREDATOR DATA
predator <- read.csv( "Bitemap_SEINE_summaries_20180825.csv", stringsAsFactors = FALSE )
...
sites <- read.csv( "Output Data/Bitemap_BioORACLE_20180825.csv", stringsAsFactors = FALSE )
...
m3na <- glmer( rate ~ poly(sstmean,3) + (1|Country), dna, family="binomial" )
# m1na <- glm( rate ~ poly(sstmean,1), dna, family="binomial" ) 
# m2na <- glm( rate ~ poly(sstmean,2), dna, family="binomial" ) 
# m3na <- glm( rate ~ poly(sstmean,3), dna, family="binomial" )
anova(m1na,m2na)
anova(m2na,m3na)
AICctab(m1na,m2na)
...
m3sp <- glmer( rate ~ poly(sstmean,3) + (1|Country), dsp, family="binomial" ) 
m4sp <- glmer( rate ~ poly(sstmean,4) + (1|Country), dsp, family="binomial" ) 
# m1sp <- glm( rate ~ poly(sstmean,1) , dsp, family="binomial" ) 
# m2sp <- glm( rate ~ poly(sstmean,2) , dsp, family="binomial" ) 
# m3sp <- glm( rate ~ poly(sstmean,3) , dsp, family="binomial" ) 
# m4sp <- glm( rate ~ poly(sstmean,4), dsp, family="binomial" ) 
anova(m1sp,m2sp)
anova(m2sp,m3sp)
anova(m3sp,m4sp)
AICctab(m1sp,m2sp)
...
## Figures for Emmett's WCMB talk
...
seagrass.color <- "#5ab4ac"    # see http://colorbrewer2.org/#type=diverging&scheme=BrBG&n=3 ALSO http://www.color-hex.com/color-palette/31668
unveg.color    <- "#d8b365"
ggplot( data=rate.env, aes(x=temp,y= rate, col=habitat,fill=habitat ) ) + 
  # geom_smooth(se=F, lwd=0.5) + 
  geom_point(pch=21,size=2) + 
  xlab("in situ temperature (°C)") + 
  ylab( expression(paste('Predation rate (',hr^-1,')'))) +
  # scale_y_log10(breaks=c(1,5,10,25,50,100)) +
  scale_fill_manual(values=c(unveg.color,seagrass.color)) + 
  scale_color_manual(values=c(unveg.color,"black")) + 
  theme_classic() +
  theme(legend.position = "none" )

ggplot( data=rate.env, aes(x=habitat,y= rate, fill=habitat ) ) + 
  # geom_smooth(se=F, lwd=0.5) + 
  geom_boxplot() +
  xlab("Habitat") + 
  ylab( expression(paste('Predation rate (',hr^-1,')'))) +
...
ggplot( data=rate.env[rate.env$rate>0,], aes(x=habitat,y= rate, fill=habitat ) ) + 
  # geom_smooth(se=F, lwd=0.5) + 
  geom_boxplot() + 
  xlab("Habitat") + 
  ylab( expression(paste('Predation rate (',hr^-1,')'))) +
...
ggplot( data=rate.env, aes(x=biomass.std,y= rate, col=habitat,fill=habitat ) ) + 
  # geom_smooth(se=F, lwd=0.5) + 
  geom_point(pch=21,size=2) + 
  xlab( "Biomass (grams per meter)" ) +
  ylab( expression(paste('Predation rate (',hr^-1,')'))) +
...
ggplot( data=rate.env, aes(x=logbio,y= rate, col=habitat,fill=habitat ) ) + 
  # geom_smooth(se=F, lwd=0.5) + 
  geom_point(pch=21,size=2) + 
  xlab( "Biomass (grams per meter)" ) +
  ylab( expression(paste('Predation rate (',hr^-1,')'))) +
...
ggplot( data=rate.env, aes(x=S.all,y= rate, col=habitat,fill=habitat ) ) + 
  # geom_smooth(se=F, lwd=0.5) + 
  geom_point(pch=21,size=2) + 
  xlab( "Predator species richness" ) +
  ylab( expression(paste('Predation rate (',hr^-1,')'))) +
  # scale_x_log10(breaks=c(1,10,100)) +
  scale_fill_manual(values=c(unveg.color,seagrass.color)) + 
  scale_color_manual(values=c(unveg.color,"black")) + 
...
ggplot( data=rate.env, aes(x=S.warm,y= rate, col=habitat,fill=habitat ) ) + 
  # geom_smooth(se=F, lwd=0.5) + 
  geom_point(pch=21,size=2) + 
  xlab( "Predator species richness" ) +
  ylab( expression(paste('Predation rate (',hr^-1,')'))) +
...
ggplot( data=rate.env, aes(x=ENSPIEfish,y= rate, col=habitat,fill=habitat ) ) + 
  # geom_smooth(se=F, lwd=0.5) + 
  geom_point(pch=21,size=2) + 
...
latm4 <- glmer( rate~abLat+sstmax + (1|Country), rateclean, family='binomial' )
...
## models with 'biology' of fish (biomass, abundance, diversity) and in situ temperature
...
bio12 <- glmer( rate~ S.warm + temp + (1|Country), rateclean, family='binomial' )
bio13 <- glmer( rate~ scale(logbio)*scale(S.warm) + scale(temp) + (1|Country), rateclean, family='binomial' )
summary(bio13)
r.squaredGLMM(bio13)
r.squaredGLMM(bio2)
r.squaredGLMM(sstm3)
bio14 <- glmer( rate~ scale(logbio)+scale(S.warm) + scale(logfish) + (1|Country), rateclean, family='binomial' )



AICctab( m0,bio1,bio2,bio3,bio4,bio5,bio6,bio7,bio8,bio9,bio10,bio11,bio12,bio13,bio14, nobs=nrow(rateclean) )


## models with fish community data
nmds1 <- glmer( rate~ MDS1 +MDS2 +MDS3 + (1|Country), rateclean, family='binomial' )
summary(nmds1)
nmds2 <- glmer( rate~ sstmean + MDS1 +MDS2  + (1|Country), rateclean, family='binomial' )
summary(nmds2)
nmds3 <- glmer( rate~ sstmean + MDS2+ MDS3 + (1|Country), rateclean, family='binomial' )
summary(nmds3)
nmds4 <- glmer( rate~ sstmean + MDS3 + (1|Country), rateclean, family='binomial' )
summary(nmds4)
nmds5 <- glmer( rate~ MDS1 + (1|Country), rateclean, family='binomial' )
summary(nmds5)
nmds6 <- glmer( rate~ temp+MDS1 + (1|Country), rateclean, family='binomial' )
summary(nmds6)
# relatinoships with SST
plot( MDS1 ~ sstmean, rateclean )
with( rateclean, cor.test( MDS1, sstmean ) )
plot( rate ~ sstmean, rateclean )
plot( rate ~ MDS1, rateclean )
ggplot( data=rateclean, aes(x=MDS1,y=rate)) + geom_point() + geom_smooth()
ggplot( data=rateclean, aes(x=sstmean,y=rate)) + geom_point() + geom_smooth()
AICctab( nmds1,nmds2,nmds3,nmds4,nmds5,nmds6, nobs=nrow(rateclean) )


### compare all models

AICctab( m0, latm3, tempm1, sstm1, sstm3,bio2, nmds1,nmds5, nobs=nrow(rateclean) )

AICctab( m0,nmds1,  tempm1,  bio2, nobs=nrow(rateclean) )
# all top models are better than intercept-only model. 
...
M3p <- glmer( rate~ scale(poly(sstmean,2)) + scale(MDS1) + (1|Site), d, family="binomial" )
summary(M3p)
...
# biomass, warm family richness, temperature
# rerun model
rate.env$scale.bio   <- scale(rate.env$logbio)
rate.env$scale.S.warm <- scale(rate.env$S.warm)
rate.env$scale.temp  <- scale(rate.env$temp)
rate.env$bin.temp    <- cut( rate.env$scale.temp, 16)
rate.env$bin.bio     <- cut( rate.env$scale.bio, 5 )
bio13 <- glmer( rate~ scale.bio*scale.S.warm + scale.temp + (1|coast/Country), rate.env, family='binomial' )
nmds1.rerun <- glmer( rate~ MDS1+MDS2+MDS3 + (1|Country), rate.env, family='binomial' )
summary(nmds1.rerun)


## model predictions for model with NMDS axes
# probably need to go one axis at a time?
## model predictions for model with biomass, warm family richness, in situ temperature
pred.frame <- with(rate.env, 
                   expand.grid( MDS1 = seq( min(MDS1,na.rm=TRUE), max(MDS1, na.rm=TRUE), len=20 ),
                                MDS2 = c( min(MDS2,na.rm=TRUE), mean(MDS2,na.rm=TRUE), max(MDS2,na.rm=TRUE) ),
                                MDS3 = mean(MDS3,na.rm=TRUE)) )
X <- model.matrix(~MDS1+MDS2+MDS3,data=pred.frame) # calculate model matrix (formula needs to be same as the model fitted)
pred <- data.frame(pred.frame,mean=(X%*%fixef(nmds1.rerun)))  # these are the point predictions
...
# make a figure
# windows(4.5,4)
ggplot( data=predictions, aes(y=logistic(mean),x=MDS1, group=MDS2, col=MDS2 )) + 
  geom_line( linewidth=2 ) + 
  # geom_line( data=predictions, aes(y=logistic(p1.lo)), linetype=2 ) + 
  # geom_line( data=predictions, aes(y=logistic(p1.hi)), linetype=2 ) + 
  geom_point( data=rate.env, aes(y=rate,x=MDS1, col=MDS2), size=2 ) +
  scale_color_viridis(s) + ylab('Predation Rate')



## model predictions for model with biomass, warm family richness, in situ temperature
pred.frame <- with(rate.env, 
                   expand.grid( scale.bio=c(-1.5,0,0.5,1),   #seq(min(scale.bio,na.rm=T),max(scale.bio,na.rm=T),len=20)
                                scale.S.warm=seq(min(scale.S.warm,na.rm=T),max(scale.S.warm,na.rm=T),len=20),
                                scale.temp=seq(min(scale.temp,na.rm=T),max(scale.temp,na.rm=T),len=20) ) )
X <- model.matrix(~scale.bio*scale.S.warm+scale.temp,data=pred.frame) # calculate model matrix (formula needs to be same as the model fitted)
pred <- data.frame(pred.frame,mean=(X%*%fixef(bio13)))  # these are the point predictions
# Calculate variance between observations within each site (for each combination of fixed effects)
V <- vcov(bio13)
pred.var1 <- diag(X %*% V %*% t(X)) # XVX^T
...
# bin temp again
predictions$bin.temp <- cut(predictions$scale.temp,16)

# try a 3D plot

cloud( logistic(mean) ~ scale.bio*scale.S.warm+scale.temp, data=predictions )
wireframe( logistic(mean) ~ scale.bio*scale.S.warm+scale.temp, data=predictions,
           shade=T )


# Several conditional plots , grouped by temperature regime
ggplot( data=predictions, aes(y=logistic(mean),x=scale.bio, group=scale.S.warm, col=scale.S.warm )) + facet_wrap( ~ bin.temp ) +
  geom_line() + 
  geom_point( data=rate.env, aes(y=rate,x=scale.bio) ) +
  scale_color_viridis() 

ggplot( data=predictions, aes(y=logistic(mean),x=scale.temp, group=scale.S.warm, col=scale.S.warm )) + 
  facet_wrap( ~ cut(scale.bio,breaks=c(-3.5,-0.75,0,0.5,1.65)), nrow = 1 ) +
  geom_line() + 
  geom_point( data=rate.env, aes(y=rate,x=scale.temp,fill=scale.S.warm), pch=21, col='black', size=3 ) +
...
summary(bio2)
# standardize predictors for std regression coefficients
bio2.scale <- glmer( rate~scale(logbio) + scale(temp) + (1|Country), rateclean, family='binomial' )
summary(bio2.scale)
exp(fixef(bio2.scale))
sd(rateclean$temp)
sd(rateclean$logbio)
# standardize based on the range (as predictor changes across its entire range, how does response change?)
lbrange <- diff(range(rateclean$logbio))
tprange <- diff(range(rateclean$temp))
rateclean <- rateclean %>% mutate( logbio.r = logbio/lbrange, temp.r = temp/tprange )
bio2.range <- glmer( rate~ logbio.r + temp.r + (1|Country), rateclean, family='binomial' ) 
summary( bio2.range )  
# linear model
bio2.r2 <- lmer( rate~scale(logbio) + scale(temp) + (1|Country), rateclean )
r.squaredGLMM(bio2.r2)
r.squaredGLMM(bio2.scale)
bio2.pr2 <- glm( rate~scale(logbio) + scale(temp), rateclean, family='binomial' )
  pscl::pR2(bio2.pr2)
...
seagrass.color <- "#5ab4ac"    # see http://colorbrewer2.org/#type=diverging&scheme=BrBG&n=3 ALSO http://www.color-hex.com/color-palette/31668
unveg.color    <- "#d8b365"
ggplot( data=rate.env, aes(x=sstmean,y= (biomass.std+1), col=habitat,fill=habitat ) ) + 
...
ggplot( data=rate.env, aes(x=sstmean,y= TotalFish.std, col=habitat, fill=habitat ) ) + 
  geom_smooth( se=F, lwd=0.5) + geom_point(pch=21,size=2, alpha=1) + 
...
predictions <- with(predictions, data.frame(predictions,
                                            ...
                                            p1.hi = mean+1.96*pred.se1))
# windows(3,3)
ggplot(rateclean, aes(x=logbio, y=rate, group=temp, col=temp )) + 
  geom_point(pch=1) + #facet_wrap(~habitat,ncol=2) +
...
pred.frame <- with(rate.env, 
                   ...
                   expand.grid( logbio=log10( c(0.5,1,5,10,25,50)*1000 ),
...
ggplot(rateclean, aes(x=temp, y=rate, group=logbio, col=logbio ) ) + 
  ...
  scale_color_viridis( ) +
  # scale_colour_gradient2(low='gray',mid="gray30",high="black", midpoint=3.5,
  #                        name="Fish\nbiomass\n(g/m)", guide="legend",
  #                        breaks=log10( c(0,1,5,10,25,50)*1000 ),
  #                        labels=c(0,1,5,10,25,50) ) +
  theme_bw() + theme( panel.grid = element_blank()  )
#
