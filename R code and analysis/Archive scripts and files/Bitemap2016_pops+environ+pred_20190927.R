...
m06 <- glmer( rate~ scale(MDS1) +# scale(MDS2)  +
               scale(temp) +  scale(log.mass.rda) + scale(log.catch.rda) +
               (1|Site), dcomp, family=fam, nAGQ=nAGQuse)
...
m22 <- glmer( rate~ scale(MDS1) +# scale(MDS2)  +
                scale(sstmean) +  scale(log.mass.rda) + scale(log.catch.rda) +
                (1|Site), dcomp, family=fam, nAGQ=nAGQuse)
...
m30 <- glmer( rate~ scale(MDS1) +# scale(MDS2)  +
                ...
                (1|Site), dcomp, family=fam, nAGQ=nAGQuse)
m31 <- glmer( rate~ scale(MDS1) +# scale(MDS2)  +
                scale(sstmean)  + scale(logpop) +
                (1|Site), dcomp, family=fam, nAGQ=nAGQuse)
m32 <- glmer( rate~ scale(MDS1) +# scale(MDS2)  +
...
m75 <- glmer( rate~ scale(MDS1) +# scale(MDS2)  +
                scale(ENSPIEfish) +
                (1|Site), dcomp, family=fam, nAGQ=nAGQuse)
m76 <- glmer( rate~ scale(MDS1) +# scale(MDS2)  +
                scale(richness.fish) +
                (1|Site), dcomp, family=fam, nAGQ=nAGQuse)
m77 <- glmer( rate~ scale(MDS1) +# scale(MDS2)  +
                 scale(logchl) +
                (1|Site), dcomp, family=fam, nAGQ=nAGQuse)
m78 <- glmer( rate~ scale(MDS1) +# scale(MDS2)  +
                scale(log.catch.rda) +
                (1|Site), dcomp, family=fam, nAGQ=nAGQuse)
m79 <- glmer( rate~ scale(MDS1) +# scale(MDS2)  +
                scale(logpop) +
                (1|Site), dcomp, family=fam, nAGQ=nAGQuse)
m80 <- glmer( rate~ scale(MDS1) +# scale(MDS2)  +
                factor(habitat) +
                (1|Site), dcomp, family=fam, nAGQ=nAGQuse)
m81 <- glmer( rate~ 
                scale(log.mass.rda) + 
...
m89 <- glmer( rate~ scale(temp)+ scale(richness.fish) +
                scale(log.mass.rda) + scale(log.catch.rda) +
                (1|Site), dcomp, family=fam, nAGQ=nAGQuse)
...
Mm <- glmer( rate~ MDS1 + MDS2  + (1|Site), d, family="binomial" )
summary(Mm)
