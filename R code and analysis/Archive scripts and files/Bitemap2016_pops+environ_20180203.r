...
pops <- pops %>%
  dplyr::select( Country, Lat, Long, dateDeployed, habitat, prop1, prop24 )
...
popmelt <- melt( pops, id.vars = 1:5, measure.vars = 6:7 )

pop <- popmelt %>%
  dplyr::select( Country, Lat, Long, dateDeployed, habitat, hour=variable, prop=value ) %>%
  mutate( hour = as.numeric(as.character(recode(hour, "prop1"=1, "prop24"=24))) )
...
zeros <- data.frame( Country=rep(sites$Country,2), 
                     ...
                     hour=0, prop=0 )
zeros <- rbind(zeros,zeros,zeros)
popall0 <- full_join(popall, zeros)


windows()
ggplot( popall000, aes(x=hour,y=prop,col=habitat) )+ facet_wrap(~Country,ncol=6) + geom_point() +
  geom_smooth( method='glm', method.args=list(family=binomial), formula=y~x, se=T ) 
library(car)
ggplot( popall, aes(x=hour,y=car::logit(prop),col=habitat) )+ facet_wrap(~Country,ncol=6) + geom_point() +
  geom_smooth( method='lm', se=F ) 

popall000 %>% filter(  )

glm1 <- glm( prop~hour*habitat, data=popall0[popall0$Country=="India",], family=binomial )
summary(glm1)
...
popsite <- ddply( popall, .(Country,sample), summarise, Lat=mean(Lat), 
