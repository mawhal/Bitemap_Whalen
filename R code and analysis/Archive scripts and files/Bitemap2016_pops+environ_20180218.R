...
# Countries 
countries <- readOGR("../Mapping", layer="ne_110m_land") 

# Winkel tripel projection
countries_wintri <- spTransform(world, CRS("+proj=wintri"))
...
ggplot( sites, aes(x=meanLat,y=sstmax) ) + geom_point() + 
...
windows(2.5,4)
ggplot( sites, aes(x=meanLat,y=sstmean) ) +  
  # geom_smooth( method='lm', formula = y~x^2, se=T ) +
  geom_smooth( aes(group=1),col='black', se=F, lwd=0.75 ) +
  geom_point(col="slateblue", alpha=0.8) +
  xlab("Degrees latitude") + 
  ylab("Mean annual sea surface\ntemperature (°C)") +
  theme_classic() +
# Flip axes to let match up with a map
 coord_flip()

# range
ggplot( sites, aes(x=abs(meanLat),y=sstrange,fill=hemi) ) + geom_point(size=2,pch=21) + 
  # geom_smooth( method='lm', formula = y~x^2, se=T ) +
...
####----------------------------------------------------------------------------------
#### model predation as a (binomial) function of time and habitat for each site
...
# add zeros?
zeros <- popall[popall$hour==1,]
zeros$hour  <- 0
zeros$eaten <- 0
zeros$prop  <- 0
zeros$noteaten <- zeros$N

popall0 <- rbind( popall,zeros,zeros,zeros )

# make factor for Country, date, and habitat
popall0$ID <- with( popall0, paste( Country,dateDeployed,habitat, sep="." ))
# 
# ggplot( popall0, aes(x=hour,y=noteaten,col=habitat) )+ facet_wrap(~Country,ncol=8) + geom_point() +
...
# predation rate data
ggplot(rate.site, aes(x=meanLat, y=rate )) + geom_point() + facet_grid(hemi~habitat) +
  geom_smooth(method='glm',method.args=list(family=quasibinomial))  

ggplot(rate.site, aes(x=meanLat, y=rate, group=hemi )) + geom_point() + facet_wrap(~habitat) +
  geom_smooth(method='glm',method.args=list(family=quasibinomial))  

ggplot(rate.site, aes(x=abs(meanLat), y=rate, group=hemi )) + geom_point() + 
  geom_smooth(method='glm',method.args=list(family=quasibinomial))  

ggplot(rate.site, aes(x=abs(meanLat), y=rate, group=hemi )) + geom_point() + 
  geom_smooth(method='glm',method.args=list(family=quasibinomial), formula=y~poly(x,2))  

ggplot(rate.site, aes(x=abs(meanLat), y=rate )) + geom_point() + 
  geom_smooth(method='glm',method.args=list(family=quasibinomial), formula=y~poly(x,2))  

# evidence for peak at warm sites, but not the warmest sites
ggplot(rate.site, aes(x=sstmean, y=rate, group=hemi )) + geom_point() + 
  geom_smooth(method='glm',method.args=list(family=quasibinomial), formula=y~poly(x,2))  
ggplot(rate.site, aes(x=sstmean, y=rate, group=habitat )) + geom_point() + facet_wrap(~hemi) +
  geom_smooth(method='glm',method.args=list(family=quasibinomial), formula=y~poly(x,2))

ggplot(rate.site, aes(x=sstrange, y=rate )) + geom_point()

ggplot(rate.site, aes(x=temp, y=rate )) + geom_point() + facet_wrap(~hemi) +
  geom_smooth(method='glm',method.args=list(family=quasibinomial))

# no difference between habitat types at this scale of analysis
ggplot(rate.site, aes(x=temp, y=rate, group=habitat )) + geom_point() + 
  geom_smooth(method='glm',method.args=list(family=quasibinomial))
ggplot(rate.site, aes(x=temp, y=rate, group=habitat )) + geom_point() + facet_wrap(~hemi) +
  geom_smooth(method='glm',method.args=list(family=quasibinomial))

windows(3,3)
ggplot( rate.site, aes(x=sstmean,y=temp) ) + geom_point() +geom_smooth() + 
  xlab("Mean annual sea surface temperature") + ylab("instantaneous temperature") +
  theme_classic() 
ggplot( rate.site, aes(x=sstmax,y=temp) ) + geom_point() +geom_smooth()


m1 <- lm(temp~sstmean,rate.site)
m2 <- lm(temp~sstmax,rate.site)
m3 <- lm(temp~sstmin,rate.site)
m4 <- lm(temp~sstrange,rate.site)
AICctab( m1,m2,m3,m4,nobs=nrow(na.omit(rate.site)) )
