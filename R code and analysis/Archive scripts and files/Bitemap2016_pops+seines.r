...
popenvsort <- popenv %>%
  dplyr::select( Country,Institution,name,email,Lat,Long,habitat,temp,sal) %>%
  plyr::arrange( Country, Lat) 


# which sites do not have in situ environmental data?
unique( popenv$Country[ is.na(popenv$temp) ] )

# look at predation data without averaging at site level
ggplot( popenv, aes(x=temp,y=prop24) ) + geom_point() +  
  geom_smooth(method='glm',method.args=list(family=binomial)) + facet_wrap(~habitat)

ggplot(popenv, aes(x=temp, y=prop24 )) + geom_point() + facet_wrap(~habitat,ncol=2) +
  geom_smooth(method='glm',  method.args=list(family=quasibinomial) ) + 
  ylab('Predation intensity\n(prop. squid missing after 24hr)') +
  xlab('in situ temperature') 

ggplot(popseinetemp, aes(x=temp, y=prop24 )) + geom_point() + facet_wrap(~habitat,ncol=2) +
  geom_smooth(method='glm',  method.args=list(family=quasibinomial) ) + 
  ylab('Predation intensity\n(prop. squid missing after 24hr)') +
  xlab('in situ temperature') 

## model all replicates using mixed effects model
allm1 <- glmer( prop24~temp+habitat + (1|Country), popenv, family='binomial' )
summary(allm1)
summary( glm( prop24~temp+habitat, popenv, family='binomial' ) )

allm2 <- glmer( prop24~temp*habitat+I(temp^2)*habitat + (1|Country), popenv, family='binomial' )
summary(allm2)

allm2 <- glmer( prop24~temp+I(temp^2) + (1|Country), popenv, family='binomial' )
summary(allm2)

allm3 <- glmer( prop24~abs(Lat)*temp + (1|Country), popenv, family='binomial' )
summary(allm3)
library(car)
vif.mer(allm3)


# residual patterns after considering one variable
mtemp <- glmer( prop24~temp + (1|Country), popenv, family='binomial' )
plot( y=resid(mtemp), x=abs(popenv$Lat[!is.na(popenv$temp)]) )


# model predictions
pred.frame <- with(popenv, expand.grid( temp=seq(min(temp,na.rm=T),max(temp,na.rm=T),len=20), 
                                        "I(temp^2)"=(seq(min(temp,na.rm=T),max(temp,na.rm=T),len=20))^2,
                                        habitat=unique(habitat) ) )
X <- model.matrix(~temp*habitat+I(temp^2)*habitat,data=pred.frame) # calculate model matrix (formula needs to be same as the model fitted)
pred <- data.frame(pred.frame,mean=(X%*%fixef(allm2)))  # these are the point predictions
# Calculate variance between observations within each site (for each combination of fixed effects)
V <- vcov(allm2)
# # model predictions
# pred.frame <- with(popenv, expand.grid( temp=seq(min(temp,na.rm=T),max(temp,na.rm=T),len=20), habitat=unique(habitat) ) )
# X <- model.matrix(~temp*habitat,data=pred.frame) # calculate model matrix (formula needs to be same as the model fitted)
# pred <- data.frame(pred.frame,mean=(X%*%fixef(allm1)))  # these are the point predictions
# # Calculate variance between observations within each site (for each combination of fixed effects)
# V <- vcov(allm1)
...
library(psych)
ggplot(popenv, aes(x=temp, y=prop24 )) + geom_point(alpha=0.2,col='slateblue') + facet_wrap(~habitat,ncol=2) +
...
AICctab( allm1,allm2, nobs=nrow(popenv[!is.na(popenv$temp),]) )


## responses to annual temperature
popSST <- left_join( pops, sites )

ggplot(popSST, aes(x=sstmean, y=prop24 )) + geom_point() + facet_wrap(~habitat,ncol=2) +
  geom_smooth(method='glm',  method.args=list(family=quasibinomial) ) + 
  ylab('Predation intensity\n(prop. squid missing after 24hr)') +
  xlab('in situ temperature') 

## model all replicates using mixed effects model
allm2 <- glmer( prop24~sstmean*habitat + (1|Country), popSST, family='binomial' )
summary(allm2)
summary( glm( prop24~temp+habitat, popenv, family='binomial' ) )

allm3 <- glmer( prop24~sstmean*habitat+I(sstmean^2)*habitat + (1|Country), popSST, family='binomial' )
summary(allm3)
vif.mer(allm3)


allm4 <- glmer( prop24~sstmean+habitat + (1|Country), popSST, family='binomial' )
summary(allm4)
vif.mer(allm4)

allm5 <- glmer( prop24~sstmean+sstrange + (1|Country), popSST, family='binomial' )
summary(allm5)
vif.mer(allm5)

# model predictions
pred.frame <- with(popSST, expand.grid( sstmean=seq(min(sstmean,na.rm=T),max(sstmean,na.rm=T),len=20), 
                                        "I(sstmean^2)"=(seq(min(sstmean,na.rm=T),max(sstmean,na.rm=T),len=20))^2,
                                        habitat=unique(habitat) ) )
X <- model.matrix(~sstmean*habitat+I(sstmean^2)*habitat,data=pred.frame) # calculate model matrix (formula needs to be same as the model fitted)
pred <- data.frame(pred.frame,mean=(X%*%fixef(allm3)))  # these are the point predictions
# Calculate variance between observations within each site (for each combination of fixed effects)
V <- vcov(allm3)
pred.var1 <- diag(X %*% V %*% t(X)) # XVX^T
...
ggplot(popSST, aes(x=sstmean, y=prop24 )) + geom_point(alpha=0.2,col='slateblue') + facet_wrap(~habitat,ncol=2) +
...
allm4 <- glmer( prop24~sstmax*habitat+I(sstmax^2)*habitat + (1|Country), popSST, family='binomial' )
summary(allm4)
allm3 <- glmer( prop24~sstmean*habitat+I(sstmean^2)*habitat + (1|Country), popSST, family='binomial' )
summary(allm3)
...
ggplot(popseine, aes(x=sstmean, y=prop24, label=site_label )) + geom_point() + geom_text(size=2.5) + 
  facet_wrap(~habitat,ncol=2) +
  geom_smooth(method='glm', formula=y~poly(x,2),method.args=list(family=binomial) ) + 
  ylab('Predation intensit\n(prop squid missing after 24hr)') +
  xlab('Annual mean SST')

# calculate average predation pressure
popsienemean <- ddply( popseine, .(Country,sstmean,site_label), summarize,
                       prop24=mean(prop24,na.rm=TRUE) )
ggplot(popsienemean, aes(x=sstmean, y=prop24, label=site_label )) +  
  geom_smooth(method='glm', formula=y~poly(x,2),method.args=list(family=binomial) ) + 
  geom_text(size=2.5) + 
  ylab('Predation intensit\n(prop squid missing after 24hr)') +
  xlab('Annual mean SST')
...
ggplot(popseine, aes(x=ENSPIE, y=prop24 )) + geom_point() + facet_wrap(~habitat,ncol=2) +
  geom_smooth(method='glm', method.args=list(family=binomial) ) + 
  ylab('Predation intensity\n(prop squid missing after 24hr)') +
  xlab('Fish diversity (Effective number of species)')



ggplot(popseine, aes(x=temp, y=log10(Total) )) + geom_point() + facet_wrap(~habitat,ncol=2) +
  geom_smooth(method='lm') 
ggplot(popseine, aes(x=log10(Total), y=prop24 )) + geom_point() + facet_wrap(~habitat,ncol=2) +
  geom_smooth(method='glm',method.args=list(family=quasibinomial)) 
ggplot(popseine, aes(x=temp, y=prop24 )) + geom_point() + facet_wrap(~habitat,ncol=2) +
  geom_smooth(method='glm',method.args=list(family=quasibinomial)) 

# temperature and fish abundance are highly correlated, and including them both in the model blows it up!
abundtemp <- glm(prop24~temp+temp:log10(Total), family=quasibinomial, popseine) 
car::vif(abundtemp)
summary( glm(prop24~temp, family=quasibinomial, popseine) )
summary( glm(prop24~log10(Total), family=quasibinomial, popseine) )
...
glm2.5 <- glm( prop24~abs(meanLat) + I(abs(meanLat)^2), popseine, family='quasibinomial' )
summary(glm2)
...
glm4.5 <- lm( log10(Total) ~ temp, popseine )
summary(glm4.5)
...
glm6 <- glm( prop24~sstmean+I(sstmean^2)+ENSPIE*habitat, popseine, family='quasibinomial' )
summary(glm6)

glm7 <- glm( prop24~log10(biomass), popseine, family='quasibinomial' )
summary(glm4)

# polynomial tests
SST1 <- glm( prop24~sstmean, popseine, family='binomial' )
SST2 <- glm( prop24~sstmean+I(sstmean^2), popseine, family='binomial' )
glm2 <- glm( prop24~abs(meanLat), popseine, family='binomial' )
glm2.5 <- glm( prop24~abs(meanLat) + I(abs(meanLat)^2), popseine, family='binomial' )
AICctab( SST1, SST2, glm2, glm2.5, nobs=nrow(popseine)-4 )

Temp1 <- glm( prop24~temp, popseine, family='binomial' )
Temp2 <- glm( prop24~temp+I(temp^2), popseine, family='binomial' )
AICctab( Temp1, Temp2, nobs=nrow(popseine)-4 )
...
# compare SST from satellites to in situ temperature
# restrict data to only those sites with in situ temp
popseinetemp <- popseine[ !is.na(popseine$temp), ]
Temp1 <- glm( prop24~temp, popseinetemp, family='binomial' )
Temp2 <- glm( prop24~temp+I(temp^2), popseinetemp, family='binomial' )
SST1 <- glm( prop24~sstmean, popseinetemp, family='binomial' )
SST2 <- glm( prop24~sstmean+I(sstmean^2), popseinetemp, family='binomial' )
AICctab( Temp1, Temp2, SST1, SST2, nobs=nrow(popseinetemp) )  ## mean annual SST better predictor
...
## compare effect size of habitat for predation, fish diversity, biomass, etc.
# Calculate habitat effect size for each site (Country)

# mixed models vs independent regressions? 
library(lme4)

# predation - remove Deleware, NSW1.1, NSW1.2, Italy
pops.mod <- pops[ !(pops$Country %in% c("Australia (NSW1.1)","Australia (NSW1.2)",
                                        "Italy","USA (DE)")), ]
pop.lmer <- lmer( prop24 ~ habitat + (habitat|Country), data=pops.mod ) 
summary(pop.lmer)
pop.effects <- data.frame(t(apply( ranef(pop.lmer)$Country, 1, function(z) z+fixef(pop.lmer) )))
names(pop.effects) <- c("pop.intercept","pop.unveg")
pop.effects$Country <- rownames(pop.effects)

# Fish diversity - remove USA (DE), USA (NC2), USA (VA)
ENSPIE.mod <- ENSPIE.seine[ !(ENSPIE.seine$Country %in% c("USA (DE)","USA (NC2)","USA (VA)")), ]
ENSPIE.mod <- droplevels(na.omit(ENSPIE.mod))
ENSPIE.lmer <- lmer( ENSPIE ~ habitat + (habitat|Country), data=ENSPIE.mod )
summary(ENSPIE.lmer)
ENSPIE.effects <- data.frame(t(apply( ranef(ENSPIE.lmer)$Country, 1, function(z) z+fixef(ENSPIE.lmer) )))
names( ENSPIE.effects ) <- c("enspie.intercept","enspie.unveg")
ENSPIE.effects$Country <- rownames(ENSPIE.effects)

# fish biomass - remove same sites as diversity
biomass.mod <- biomass_total[ !(biomass_total$Country %in% c("USA (DE)","USA (NC2)","USA (VA)")), ]

# combine effects
effects <- full_join( pop.effects, ENSPIE.effects )
plot( pop.unveg~enspie.unveg, effects )
#


# independent linear models, using lme4
ENSPIE.list <- lmList( ENSPIE~habitat|Country, ENSPIE.mod )
ENSPIE.ind  <- coef(ENSPIE.list)
names(ENSPIE.ind) <- c("enspie.intercept","enspie.unveg")
ENSPIE.ind$Country <- rownames(ENSPIE.ind)

Biomass.list <- lmList( biomass~habitat|Country, biomass.mod )
Biomass.ind  <- coef(Biomass.list)
names(Biomass.ind) <- c("biomass.intercept","biomass.unveg")
Biomass.ind$Country <- rownames(Biomass.ind)

pred.list   <- lmList( prop24~habitat|Country, pops.mod )
pred.ind  <- coef(pred.list)
names(pred.ind) <- c("pop.intercept","pop.unveg")
pred.ind$Country <- rownames(pred.ind)

# combine effects
effects <- full_join( pred.ind, ENSPIE.ind  )
effects <- full_join( effects, Biomass.ind  )
plot( pop.unveg~enspie.unveg, effects )
summary( lm(pop.unveg~enspie.unveg, effects) )
plot( pop.unveg~biomass.unveg, effects )



#
windows(4,7)
ggplot( pops.mod, aes(x=as.numeric(habitat),y=prop24)) + geom_point() + 
  geom_smooth(method='lm') + facet_wrap(~Country)
ggplot( pop.mean, aes(x=habitat,y=prop24)) + geom_point() + 
  geom_line(aes(group=Country)) + facet_wrap(~Country)
ggplot( ENSPIE.mod, aes(x=as.numeric(habitat),y=ENSPIE)) + geom_point() + 
  geom_smooth(method='lm') + facet_wrap(~Country)
ggplot( ENSPIE.site, aes(x=habitat,y=ENSPIE)) + geom_point() + 
  geom_line(aes(group=Country)) + facet_wrap(~Country) 
ggplot( biomass_total, aes(x=as.numeric(habitat),y=biomass)) + geom_point() + 
  geom_smooth(method='lm') + facet_wrap(~Country)
