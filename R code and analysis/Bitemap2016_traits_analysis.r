...
# read seine data with consumers based on eat.squid category
pred_biom <- read_csv( "Output Data/consumer_biomass_seine.csv" )
pred_biom$sciName <- gsub( " ", ".", pred_biom$SPECIES_NAME  )
# ## read seine data
...
# seine$family[ grep('Hyporhamphus*',seine$SPECIES_NAME,TRUE) ] <- 'Hemiramphidae'
# seine$eat.squid[ grep('Hyporhamphus*',seine$SPECIES_NAME,TRUE) ] <- 1
# seine$family[ grep('Arripis*',seine$SPECIES_NAME,TRUE) ] <- 'Arripidae'
# seine$eat.squid[ grep('Arripis*',seine$SPECIES_NAME,TRUE) ] <- 1
# seine$family[ grep('Gadus*',seine$SPECIES_NAME,TRUE) ] <- 'Gadidae'
# seine$eat.squid[ grep('Gadus*',seine$SPECIES_NAME,TRUE) ] <- 1
# seine$family[ grep('Tozeuma*',seine$SPECIES_NAME,TRUE) ] <- 'Hippolytidae'
# seine$eat.squid[ grep('Tozeuma*',seine$SPECIES_NAME,TRUE) ] <- 0
# seine$family[seine$SPECIES_NAME == 'Cancer irroratus'] <- 'Cancridae'
# seine$eat.squid[ seine$family=="Cancridae" ] <- 1
...
## select columns
trait <- trait %>%
  select( Country, family, sciName, active.ambush, feedingImpute, 
          trophicImpute, waterColumnImpute, eat.squidImpute,
          BodyShapeI, BodyShapeII, Forehead, TypeofMouth,
          PosofMouth, CShape, Attributes, TL, HL, BD, SnoutTipX, SnoutTipY,
          AspectRatio, EncIndex, DietTroph, DietSeTroph, FoodTroph,FoodSeTroph, 
          # SoftBottom, Sand, Mud, HardBottom,SeaGrassBeds,
          # CoralReefs, Estuaries, Mangroves, Intertidal, Saltmarshes, 
          Herbivory2,  Climate, FoodI, FoodII) %>%
  distinct()
trait$sciName <- gsub( " ", ".", trait$sciName  )
x <- trait %>% select( Country, family, sciName )
x2 <- x %>% distinct
xdup <-  which( duplicated( x ) )
# trait <- trait[ -xdup, ]
# seine <- pred_biom %>%
#   select( Country, Lat, Long, habitat, Depth, Distance, width.transect, phylum, family, sciName=SPECIES_NAME, 
#           Length, Abundance, eat.squid )
# seine$sciName <- gsub( " ", ".", seine$sciName  )
# change family of Carcinus maeanas
# pred_biom$family[ pred_biom$sciName=="Carcinus maenas"] <- "Carcinidae"
sites <- sites %>%
  select( Country, Site, basin, coast, meanLat, meanLong, chlomax, chlomean, chlomin, chlorange, cloudmax, cloudmean, cloudmin,
          parmax, parmean, salinity, sstmax, sstmean, sstmin, sstrange, rate )

## merge
s1 <- left_join( pred_biom, sites )
s1<- left_join( s1, trait )

s1[is.na(s1$eat.squid),]

# duplicate entries?
trait %>%
  arrange( Country, sciName ) %>%
...
suse <- filter( s1, phylum=="Chordata" )
suse2 <- left_join( distinct(select(s1,Country,Site)), suse )
# ratio of active to ambush at each site by individual 
rat.ind <- suse2 %>%
  dplyr::group_by( Site ) %>%
  dplyr::summarize(act.ratio.ind = length(active.ambush[active.ambush %in% c("active","ambush.active")]) / length(sciName)  ) 
# and by taxon
rat.tax <- suse2 %>%
  dplyr::select( Site, sciName, active.ambush ) %>%
  dplyr::distinct() %>%
  dplyr::group_by( Site ) %>%
  dplyr::summarize( act.ratio.tax = length(active.ambush[active.ambush %in% c("active","ambush.active")]) / length(sciName),
                    richness=length(sciName)) 
# merge
rat <- left_join( rat.ind, rat.tax )
rat.site <- left_join( rat, sites )
# plot
...
suse %>% 
  filter( Site %in% hilo ) %>% 
  select( Site, phylum, family, SPECIES_NAME ) %>% 
  distinct()
...
Mode( c(0,1, NA))
# first convert benthopelagic to demersal
trait$waterColumnImpute[ trait$waterColumnImpute == "benthopelagic"] <- 'demersal'
trait.mode <- trait %>%
...
class(trait.mode$TL)
trait.mode.clean <- trait.mode
trait.mode.clean[,sapply( trait.mode, class  ) == "character"] <- data.frame( 
  apply(trait.mode.clean[,sapply( trait.mode.clean, class  ) == "character"], 2, clean) 
  )
# trait.mode.clean[,15:35] <- apply(trait.mode.clean[,14:34],2,as.numeric)
...
xuse <- trait.keep[,-c(1,6)]
# correct NA values
xuse[xuse==""] <- NA
summary(xuse)
  nacount <- apply( xuse, 2, function(z) length(which(is.na(z))) )
# get rid of traits missing more than half of the entries
xuse <- xuse[ ,nacount<nrow(xuse)/2 ]
# xuse <- xuse[,-c(23:32)] #habitats
row.names(xuse) <- trait.keep$sciName
...
xmat <- list( trait=xuse, abun=as.matrix(buse) )
debugonce(FD::dbFD)
x <- as.list( body(dbFD) )
as.list( x[[54]] )
...
tr.clust <- cascadeKM(traits, km.inf.gr, km.sup.gr, 
                      km.iter, km.crit)
m1 <- dbFD( xmat$trait, xmat$abun, w.abun = T, 
            corr="lingoes", print.pco=T, calc.FGR = T, clust.type="kmeans",
            km.inf.gr=3,km.sup.gr=20,
            calc.CWM = FALSE, m="min", calc.FDiv = FALSE )
# m1 <- dbFD( xmat$trait, xmat$abun, w.abun = T, 
#             corr="lingoes", print.pco=T, calc.FGR = T)
dbFD( xmat$trait, xmat$abun,corr="lingoes" )
...
# group abundance
gr.abun <- m1$gr.abun
gr.abun$Site <- rownames(gr.abun)
gr.abun <- left_join( gr.abun, sites )
# can we define groups based on trait bundles?
# taxa in each group
groups <- data.frame( sciName=names(m1$spfgr), functgroup=m1$spfgr )
seine.group <- left_join( seine.keep, groups )
...
group.site <- seine.group %>%
  dplyr::mutate( area=Distance*width.transect ) %>% 
  dplyr::group_by( Site, sciName, functgroup,area ) %>%
  dplyr::summarize( n=sum(Abundance,na.rm=T))
group.long <- group.site %>%
  dplyr::ungroup() %>%
  dplyr::select( Site, functgroup ) %>%
  dplyr::group_by( Site, functgroup ) %>%
  dplyr::summarize( count=length(functgroup) ) #%>%
# spread( functgroup, count, fill=0 )
# names(group.wide)[-1] <- paste0("group",names(group.wide)[-1])
group.rate <- right_join( sites, group.long )
...
group.n <- group.site %>%
  dplyr::ungroup() %>%
  dplyr::mutate( cpua=n/area ) %>%
  dplyr::select( Site, functgroup, cpua ) %>%
  dplyr::group_by( Site, functgroup ) %>%
